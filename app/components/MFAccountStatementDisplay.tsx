...
interface MFAccountStatementDisplayProps {
  data: any;
}

export default function MFAccountStatementDisplay({ data }: MFAccountStatementDisplayProps) {
  const [expandedTransactions, setExpandedTransactions] = useState<Set<number>>(new Set());
  const [filterType, setFilterType] = useState<'ALL' | 'CREDIT' | 'DEBIT' | 'BUY' | 'SELL'>('ALL');
  ...
  if (!data) {
    return (
      <div className="text-center py-12 text-dark-textSecondary">
        ...
        <p className="text-lg">No mutual fund account statement data available</p>
      </div>
    );
  }
  ...
  const renderField = (key: string, value: any) => {
    ...
    if (lowerKey.includes('amount') || lowerKey.includes('value') || lowerKey.includes('price') || lowerKey.includes('nav') || lowerKey.includes('units') || lowerKey.includes('balance')) {
  ...
  const totalCredits = filteredTransactions
    .filter((txn: any) => (txn.type || txn.transactionType || '').toUpperCase().includes('CREDIT') || (txn.type || txn.transactionType || '').toUpperCase().includes('BUY'))
    .reduce((sum: number, txn: any) => sum + (parseFloat(txn.amount || txn.value || 0) || 0), 0);
  
  const totalDebits = filteredTransactions
    .filter((txn: any) => (txn.type || txn.transactionType || '').toUpperCase().includes('DEBIT') || (txn.type || txn.transactionType || '').toUpperCase().includes('SELL'))
    .reduce((sum: number, txn: any) => sum + (parseFloat(txn.amount || txn.value || 0) || 0), 0);
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <p className="text-xs text-dark-textSecondary">Total Credits/Buy</p>
              <p className="text-2xl font-bold text-accent-success">{formatCurrency(totalCredits)}</p>
            </div>
          </div>
        </motion.div>
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            <span className="text-3xl">⬆️</span>
            <div>
              <p className="text-xs text-dark-textSecondary">Total Debits/Sell</p>
              <p className="text-2xl font-bold text-accent-danger">{formatCurrency(totalDebits)}</p>
            </div>
          </div>
        </motion.div>
      </div>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1">
            <div className="flex items-center gap-3">
              ...
              <input
                type="text"
                placeholder="Search transactions, ISIN, narration, reference..."
                value={searchQuery}
      ...
      {filteredTransactions.length === 0 ? (
        ...
        <div className="space-y-4">
          {filteredTransactions.map((txn: any, idx: number) => {
            ...
            const isCredit = txnType.includes('CREDIT') || txnType.includes('BUY');
            const amount = parseFloat(txn.amount || txn.value || 0);
            ...
            return (
              <motion.div
                ...
                className={`glass-effect rounded-xl p-6 border-2 ${
                  isCredit ? 'border-accent-success/30' : 'border-accent-danger/30'
              ...
              >
                ...
                >
                  <div className="flex items-center justify-between gap-4 mb-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <span className={`text-2xl ${isCredit ? 'text-accent-success' : 'text-accent-danger'}`}>
                          {isCredit ? '⬇️' : '⬆️'}
                        </span>
                        <div>
                          <h4 className="text-lg font-bold text-dark-text">
                            {txn.narration || txn.description || txn.fundName || 'Transaction'}
                          </h4>
                          {txn.isin && (
                      ...
                      <div className="flex flex-wrap gap-2">
                        <span className={`text-xs px-2 py-1 rounded ${
                          isCredit ? 'bg-accent-success/20 text-accent-success' : 'bg-accent-danger/20 text-accent-danger'
                        }`}>
                          {txnType}
                        </span>
                    ...
                    <div className="text-right">
                      ...
                      {txn.balance !== undefined && (
                        <p className="text-xs text-dark-textSecondary mt-1">Balance: {formatCurrency(txn.balance)}</p>
                      )}
